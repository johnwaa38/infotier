...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (typeof header === 'string' && header.startsWith('Bearer ')) {
      ...
      const ownerOnly = this.reflector.getAllAndOverride<boolean>(OWNER_ONLY_KEY, [context.getHandler(), context.getClass()]);
      if (ownerOnly && request.user.customerRole !== 'owner') throw new UnauthorizedException('Owner authentication required');
      return true;
    }
    ...
    if (typeof apiKey === 'string') {
      ...
      if (principal) {
        ...
        const ownerOnly = this.reflector.getAllAndOverride<boolean>(OWNER_ONLY_KEY, [context.getHandler(), context.getClass()]);
        if (ownerOnly && principal.customerRole !== 'owner') throw new UnauthorizedException('Owner authentication required');
